...
for(let k = 0; k < tuglaSutunSayisi; k++){
    ...
    for(let s = 0; s < tuglaSatirSayisi; s++){
        let surpriz = false;
        if (surprizVerildi < surprizSayisi && Math.random() < 0.1) { 
            surpriz = true;
            surprizVerildi++;
        }
...
const topunKonumunuDegistir = () => {
    ...
    if(y + dy < 10){ 
        ...
        y + dy > cubukY - 10 && y + dy < cubukY + 10 && 
        x > cubukX && x < cubukX + cubukGenisligi 
    ) {
        dy = -dy;
    ...
    for (let i = 0; i < dusenSuprizler.length; i++) {
        ...
        if (supriz.status === 1) {
            ...
            if (supriz.y >= cubukY && supriz.y <= cubukY + cubukYuksekligi &&
                ...
                const buyut = Math.random() >= 0.5;
            
                if (buyut && cubukGenisligi <= 100) {
                    cubukGenisligi =  cubukGenisligi * 2;
                } else if (!buyut && cubukGenisligi >= 20) {
                    cubukGenisligi = cubukGenisligi /2;
                }
            }
        }
...
const skoruCiz = () => {
    ctx.font = "16px Arial";
    ctx.fillStyle = "#0000FF";
    ctx.fillText(`Puanınız: ${skor}`,8,20)
}
...
function myKeydownFunction(e){
    if(e.key === "Right" || e.key === "ArrowRight"){
        if(cubukX + 50 > width - cubukGenisligi) return;
